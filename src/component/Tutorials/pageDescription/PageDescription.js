...
const PageDescription = forwardRef((props, ref) => {
    ...
    <dl key={key}>
      <dt
        onClick={() => {
          ...
          getDetails(`${config.baseURL}${data.content[key].location}`);
  ...
  const prevButtonHandler = (urlForPrev) => {
    ...
    getDetails(`${config.baseURL}${data.content[curPage - 2].location}`);
  ...
  const nextButtonHandler = (urlForNext) => {
    ...
    getDetails(`${config.baseURL}${data.content[curPage].location}`);
